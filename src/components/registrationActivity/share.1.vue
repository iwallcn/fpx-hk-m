<template>
  <div class="register">
    <div class="wrapper">
      ...
      <div class="agree">
        <span class="first"
          @click="handlerAgree"><i :class="['iconfont', isAgree ? 'icon-check' : 'icon-nocheck']"></i>我已閱讀并接受</span><span @click="popupVisible = true">《4PX注冊服務協定》</span>
      </div>
      <div class="btn">
    ...
    <img src="../../assets/image/register_bg2.jpg">
    ...
    <div v-transfer-dom>
      <popup v-model="popupVisible"
        ...
        position="bottom"
        width="100%">
        <Agreement @close="popupVisible = false"></Agreement>
      </popup>
    </div>
  </div>
</template>
    ...
    handlerRegister () {
      ...
      API.userRegistrationActivity(data).then(res => {
        if (res.code === this.GLOBAL.code) {
          ...
          this.$router.push({ name: 'shareOK', query: { areaCode: this.registerData.areaCode } })
