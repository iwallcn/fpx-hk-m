<template>
  <div class="register">
    <div class="wrapper">
      ...
      <div class="row2">
        <x-input v-model="registerData.verificationKey"
          placeholder="請輸入驗證碼">
          <x-button slot="right"
            type="primary"
            class="btn"
      ...
      <div class="btn">
        <button @click="handlerRegister">點擊註冊</button>
      </div>
      <div class="goLogin">
...
<script>
import { PopupPicker, Popup, XInput, XButton } from 'vux'
...
export default {
  ...
  created () {
    // this.type = this.$route.params.type // 获取动态路由参数
  ...
  methods: {
    // 获取活动背景图片
    getBackgroundImg () {
      API.backgroundImgQuery({ url: this.$route.params.type }).then(res => {
        if (res.code === this.GLOBAL.code && res.data) {
          this.backgroundImg = res.data[0].baseMap
          this.registChannel = res.data[0].registChannel
        }
      })
    },
    showProtocol () {
