...
export default {
  ...
  created () {
    ...
    if (this.editAdd === 'edit') {
      this.obj = this.returnAddress()
      this.areaCode = [this.obj.areaCode]
    }
  },
  methods: {
    // 上传说明
    ...
    isNull () {
      if (this.obj.firstName.trim() && this.obj.phone.trim() && this.obj.province && this.obj.detailAddress) {
        this.disabled = false
      } else {
        this.disabled = true
      }
