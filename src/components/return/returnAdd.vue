...
import { queryPgk, setReturnPackagePrealert } from '@/api/return'
...
export default {
  ...
  beforeRouteEnter (to, form, next) { // 因为当钩子执行前，组件实例还没被创建
    if (['returnDetail', 'account', 'returns'].indexOf(form.name) >= 0) { // 从退件详情，我的退件，或者个人中心进来清空表单
      next(vm => {
        vm.searchCode = ''
        vm.packageGoodsDTOList = []
        vm.returnPackageData.packagePrealertGoodsVOS = ''
        vm.pickupAddressCheck = {}
        vm.returnAddressCheck = {}
        vm.isAgree = false
        vm.SET_RETURN_ADDRESS({})
      })
    } else {
      next()
    }
