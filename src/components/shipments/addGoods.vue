<template>
  <div class="add-goods">
    <div class="history">
      <span @click="loadProducts">歷史商品記錄</span>
    </div>
    <group gutter=".2rem"
      label-width="1.8rem"
      ...
      <x-input v-model.trim="goodsData.goodsName"
        :disabled="!isEncrypted && hasData"
        :placeholder="goodsData.originPlace === 'JP' ? '請輸入商品日文名稱' : '請輸入名稱'">
        <div slot="label"
          class="cell-label">
          <span class="cell-must">*</span>
          <span class="cell-name"
            v-html="goodsData.originPlace === 'JP' ? '日文名稱' : '名稱'"></span>
        </div>
      </x-input>
      <x-input v-model.trim="goodsData.brand"
        :disabled="!isEncrypted && hasData"
        placeholder="請輸入品牌">
        <div slot="label"
          class="cell-label">
          <span class="cell-must">*</span>
          <span class="cell-name">品牌</span>
        </div>
      </x-input>
      <template v-if="!isEncrypted && hasData">
        <x-input v-model.trim="goodsData.categoryName"
          disabled>
          <div slot="label"
            class="cell-label">
            <span class="cell-must">*</span>
      ...
      <template v-if="!isEncrypted && hasData">
        <cell :value="goodsData._originPlace">
          <div slot="title"
            class="cell-label">
            <span class="cell-must">*</span>
            <span class="cell-name"
              style="color: #999;">產地</span>
          </div>
        </cell>
      </template>
      <template v-else>
        <popup-picker class="add-popup"
          ...
          <div slot="title"
            class="cell-label">
            <span class="cell-must">*</span>
            <span class="cell-name">產地</span>
          </div>
        </popup-picker>
      </template>
      <x-input v-model.trim="goodsData.spec"
        :disabled="!isEncrypted && hasData"
        placeholder="請輸入規格型號">
        <div slot="label"
          class="cell-label">
          <span class="cell-must">*</span>
          <span class="cell-name">規格型號</span>
        </div>
      </x-input>

      <template v-if="!isEncrypted && hasData">
        <x-input v-model.trim="goodsData.number"
          disabled>
          <div slot="label"
            class="cell-label">
            <span class="cell-must">*</span>
      ...
      <x-input ref="upcInput"
        ...
        <div slot="label"
          class="cell-label">
          <span class="cell-must">&nbsp;&nbsp;</span>
          <span class="cell-name">UPC碼</span>
        </div>
      </x-input>
    </group>
    ...
    <div v-transfer-dom>
      <popup v-model="categoryShow"
        ...
        position="bottom"
        height="50%">
        <Category @close="categoryShow = false"
  ...
  methods: {
    ...
    getData () {
      ...
      if (this.isGoodsEdit) {
        ...
        this.goodsData = data
        this.goodsData._originPlace = [data.originPlace]
      }
    },
    // 打开品类选择弹窗
    handleGoodsCategory () {
