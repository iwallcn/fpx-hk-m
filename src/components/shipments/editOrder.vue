  ...
  <div class="order">
    ...
    <group>
      ...
      <popup-picker title="啟運倉:"
        ...
        :disabled="$route.name === 'editOrder'"
        is-link
        :class="$route.name === 'editOrder' ? 'disabled' : ''"
...
export default {
  ...
  activated () {
    // 设置请求token
    this.getQequestToken()
    console.log('activated', this.$route.query.fpxOrderId, this.fpxOrderId)
        ...
        oAPI.getPackagePrealert(fpxOrderId).then(res => {
          if (res.code === this.GLOBAL.code) {
            ...
            if (res.data.packagePrealertAddress && res.data.packagePrealertAddress.deliveryType !== undefined) {
              this.getProductConfigList(res.data.packagePrealertAddress.deliveryType)
            }
            // 增值服务
      ...
      if (this.isMerge) {
        if (this.$route.name === 'editOrder') {
          ...
          let mergeData = {
            ...
            warhouseCode: data.warhouseCode,
            packagePrealertGoodsVOS: data.packagePrealertGoodsVOS,
            referOrderId: data.referOrderId,
            fpxOrderId: data.fpxOrderId
          }
          oAPI.setUpdatePackagePrMerge(mergeData).then(res => {
            if (res.code === this.GLOBAL.code) {
              let msg = '修改成功'
