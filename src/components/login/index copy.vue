...
export default {
  ...
  created () {
    ...
    // this.$store.state.thirdLoginFlag
  },
  methods: {
    // 标记登录类型
    setLocal (val) {
    ...
    handleSubmit () {
      ...
      axios.post(utils.getApiUrl() + '/mobileTerminalLogin/mlogin', data, { // 正常
        ...
        if (res.data.errCode == 40001) {
          this.$vux.toast.text('驗證碼錯誤')
          this.sendGaEvent(0, '验证码错误')
        }
        if (res.data.errCode == 40002) {
          this.$vux.toast.text('輸入密碼錯誤次數過多，此帳號暫時被鎖定')
          this.sendGaEvent(0, '输入密码错误次数过多，此账号暂时被锁定')
        }
        if (res.data.errCode == 40003) {
          if (res.data.msg.indexOf('Non-existent') !== -1) {
            this.$vux.toast.text('賬號不存在，請重新輸入')
            this.sendGaEvent(0, '账号不存在')
          } else {
            this.$vux.toast.text('密碼錯誤，請重新輸入')
            this.sendGaEvent(0, '密码错误')
          }
        ...
        if (res.data.errCode == 40008 || res.data.errCode == 40009) {
          this.$vux.toast.text('賬號異常，請聯繫客服人員')
          this.sendGaEvent(0, '账号异常，请联系客服人员')
        }
        if (res.data.errCode == null) {
          if (!res.data.data) {
            this.$vux.toast.text(res.data.msg)
            this.sendGaEvent(0, res.data.msg)
            return
          }
          let tkVal = res.data.data.ticket
          if (!tkVal) {
            this.$vux.toast.text('生產票據失敗')
            this.sendGaEvent(0, '生产票据失败')
            return
          }
          utils.setCookie('TGC', res.data.data.TGC, 8)
          utils.setStorage('TGC', res.data.data.TGC)
          this.sendGaEvent(1, '登录成功')
          let fservice = this.$route.query.fservice
          if (fservice) {
            if (fservice.indexOf('?') !== -1) {
              window.location.href = fservice + '&' + tkVal + '&_=' + new Date().getTime()
            } else {
              window.location.href = fservice + '?' + tkVal + '&_=' + new Date().getTime() + '&serviceUrl=' + location.origin + this.serviceUrl
            }
          } else {
            if (service.indexOf('?') !== -1) {
              window.location.href = service + '&' + tkVal + '&_=' + new Date().getTime()
            } else {
              window.location.href = service + '?' + tkVal + '&_=' + new Date().getTime() + '&serviceUrl=' + location.origin + this.serviceUrl
            }
          }
        }
      }).catch((err) => {
