...
import { insertEmailCode, emailNewVerification } from '@/api/personalInfo'
export default {
  name: 'setemail',
  components: {
    Flow,
  ...
  computed: {
    isActive () {
      if (!this.obj.email || !this.obj.verificationCode || !this.obj.validateCode) {
        return false
      } else {
        return true
      }
  ...
  methods: {
    ...
    getVerificationKey () {
      ...
      this.sendFlag = true
      insertEmailCode(this.obj).then(res => {
