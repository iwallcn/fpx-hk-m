  ...
  <div class="popup-address"
    ...
    <div class="popup-content">
      ...
      <div class="popup-body">
        ...
        <ul class="popup-ul">
          <li v-for="(prov, provIndex) in provinceList['460']"
            :key="prov.code"
            v-show="showProvince"
...
export default {
  ...
  methods: {
    setClick () {
      ...
      if (that.province && p && p[HKCODE]) {
        that.clickPro = (p[HKCODE].filter((item) => item.nameChinese === that.province))[0]
    ...
    getProvince () {
      ...
      if (!this.provinceData()) {
        getChildArea(HKCODE).then(res => {
          if (res.code === this.GLOBAL.code) {
            this.provinceList[HKCODE] = res.data
